...
import 'package:bloc_app/shared/ui/stacked_snack_bar_manager.dart';
...
class _HomeState extends State<Home> {
  ...
  Widget build(BuildContext context) {
    return StackedSnackBarManager(
      child: BlocConsumer<HomeBloc, HomeState>(
        bloc: homeBloc,
        listenWhen: (previous, current) => current is HomeActionState,
        buildWhen: (previous, current) => current is! HomeActionState,
        listener: (context, state) {
          final stackedSnackBarManager = StackedSnackBarManager.of(context);
          if (state is HomeNavigateToCartPageActionState) {
            ...
            stackedSnackBarManager
                ?.showMessage("${state.clickedProduct.name} added to cart.");
